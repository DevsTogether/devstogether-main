...
export interface ICan {
    ...
    userId?: string;
    context?: unknown;
    action: "see-public-reports" | "see-private-reports" | "create-question" | "create-comment";
}
...
class Auth {
    ...
    async can(options: ICan): Promise<boolean> {
        const user = options.user || await prisma.user.findUnique({ where: { id: options.userId } });
        const checkers: { [key: string]: Function } = {
            "see-public-reports": (user: User, context: unknown): boolean => true,
            "see-private-reports": (user: User, context: unknown): boolean => {
                return (
                    !user.banned &&
                    user.roleId === constants.USER_ROLES.ADMIN
                );
            },
            "create-comment": (user: User, context: unknown): boolean => {
                return (
                    !user.banned &&
                    user.reputation >= constants.MIN_REPUTATION_TO_CREATE_COMMENT
                );
            },
        }
